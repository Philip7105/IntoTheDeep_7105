...
public class PushSamplesAuto extends BaseAuto {
    ...
    public void runAuto() {
        ...
        runpath = new MultipleRRActionsWithPathing(new Action[]{new SequentialAction(groups.moveClipMagAction(ClipMech.ArmStates.READY),groups.moveVerticalSlidesAction(highchamber, greaterTolerance)), robot.driveTrain.strafeToLinearHeading(new Vector2d(-9.5, 63), Math.toRadians(90), new Vector2d(-6, 32.4), Math.toRadians(90))})
                ...
                .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(-6, 32.4), Math.toRadians(90), new Vector2d(-35,38),Math.toRadians(90))
                        ,new SequentialAction(groups.moveVerticalSlidesAction(0, greaterTolerance),groups.moveClipMagAction(ClipMech.ArmStates.OUT_THE_WAYOFHORIZONTALSLIDES)),groups.moveIntakeActionForShortTime(NewIntake.IntakeStates.STOP)}))
                .addNext(new MultipleRRActionsConstraints(new Action[]{robot.driveTrain.getTheSamplesBetter(new Vector2d(-35,38),Math.toRadians(90))},10,3));
//                .addNext();
                ...
                /*
                .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(-6, 32.4), Math.toRadians(90), new Vector2d(-43.8, 56.7), Math.toRadians(90))
