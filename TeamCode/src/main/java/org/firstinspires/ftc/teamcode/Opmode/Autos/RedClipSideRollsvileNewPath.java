...
public class RedClipSideRollsvileNewPath extends BaseAuto {
    ...
    public void runAuto() {
        ...
        runpath = new MultipleRRActionsWithPathing(new Action[]{groups.delayAction(.3,groups.moveVerticalSlidesAction(lowchamber, greaterTolerance)),robot.driveTrain.strafeToLinearHeading(new Vector2d(-9.5,63), Math.toRadians(90),new Vector2d(-4,31),Math.toRadians(90)), groups.moveClipMagAction(ClipMech.ArmStates.OUT_THE_WAY)})
                .addNext(new Delay(.2))
