...
public class MecanumDrive {
    public static double errortolerance = 3, velocitytolerance = 2;
    public static class Params {
        ...
        public double axialVelGain = 0.000000000002;
        public double lateralVelGain = 0.000000000002;
...
//            p.put("headingError (deg)", Math.toDegrees(error.heading.toDouble()));
    ...
    public class FollowTrajectoryActionBetter implements Action {
        public final TimeTrajectory timeTrajectory;
        private double beginTs = -1;

        private final double[] xPoints, yPoints;

        public FollowTrajectoryActionBetter(TimeTrajectory t) {
            timeTrajectory = t;
        ...
        public boolean run(TelemetryPacket p) {
            ...
            Pose2d error = txWorldTarget.value().minusExp(pose);
// TODO: Check to make sure that this works.
            if ((t >= timeTrajectory.duration && error.position.norm() < errortolerance
                    && robotVelRobot.linearVel.norm() < velocitytolerance) || t >= timeTrajectory.duration + 1) {
                leftFront.setPower(0);
                leftBack.setPower(0);
                rightBack.setPower(0);
                rightFront.setPower(0);
...
//            p.put("headingError (deg)", Math.toDegrees(error.heading.toDouble()));
    ...
    public TrajectoryActionBuilder actionBuilderBetter(Pose2d beginPose) {
        return new TrajectoryActionBuilder(
                TurnAction::new,
                FollowTrajectoryActionBetter::new,
                new TrajectoryBuilderParams(
                        1e-6,
                        new ProfileParams(
                                0.25, 0.1, 1e-2
