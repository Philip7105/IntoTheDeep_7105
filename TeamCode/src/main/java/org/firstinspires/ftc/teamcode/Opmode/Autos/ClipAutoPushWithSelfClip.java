...
import org.firstinspires.ftc.teamcode.robot.Commands.RoadRunnerActions.MultipleRRActionsWithConstraint;
...
public class ClipAutoPushWithSelfClip extends BaseAuto{
        ...
        public void runAuto() {
            ...
            runpath = new MultipleRRActionsWithPathing(new Action[]{groups.delayAction(.3,groups.moveVerticalSlidesAction(highchamber, greaterTolerance)),robot.driveTrain.strafeToLinearHeading(new Vector2d(-9.5,63), Math.toRadians(90),new Vector2d(-2,31),Math.toRadians(90)), groups.moveClipMagAction(ClipMech.ArmStates.OUT_THE_WAY)})
                    .addNext(new Delay(.2))
                    ...
                    // go to clip
                    .addNext(new MultipleRRActionsWithConstraint(new Action[]{robot.driveTrain.getTheSamples(new Vector2d(-2 ,31), Math.toRadians(90))
                            , groups.moveVerticalSlidesAction(clipoffwall, greaterTolerance)
                            ,groups.moveClipMagAction(ClipMech.ArmStates.OUTOFTHEWAYFORAUTO),
                    ...
                    .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(-62.5, 52), Math.toRadians(90),new Vector2d(-62.5, 54), Math.toRadians(90))
                            ,groups.moveHorizontalSlidesAction(HorizontalSlides.HorizontalSlideStates.HALF_OUT, halfOutEncoderPos),
                            groups.movePivotAction(JohnsIntake.PivotStates.PARALLEL),
                            groups.moveGripperAction(JohnsIntake.GripperStates.UNCLAMP),
                            groups.moveIntakeActionForShortTime(JohnsIntake.IntakeStates.INTAKESLOW)}));
//                    .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(-62.5, 54), Math.toRadians(90),new Vector2d(-62.5, 50), Math.toRadians(90))
//                            ,groups.moveHorizontalSlidesAction(HorizontalSlides.HorizontalSlideStates.HALF_OUT, halfOutEncoderPos),
//                            groups.movePivotAction(JohnsIntake.PivotStates.PARALLEL),
//                            groups.moveGripperAction(JohnsIntake.GripperStates.UNCLAMP),
//                            groups.moveIntakeActionForShortTime(JohnsIntake.IntakeStates.INTAKESLOW)}));
