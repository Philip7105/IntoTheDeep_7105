...
public class BlueBucket3 extends BaseAuto {
    ...
    public void runAuto() {
        ...
        runpath = new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(32, 60), Math.toRadians(180),new Vector2d(48.2,47),Math.toRadians(90))})
                .addNext(new Delay(2))
                ...
                .addNext(new Delay(2))
                .addNext(new MultipleCommand(groups.moveGripper(JohnsIntake.GripperStates.clamp),groups.moveIntake(JohnsIntake.IntakeStates.stop), groups.moveHorizontalSlides(Fully_In, fullin)))
                .addNext(new MultipleCommand(groups.slidesSetPos(lowbasket), groups.movePivotJohn(JohnsIntake.PivotStates.basketpos)))
                .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(48.2,47),Math.toRadians(90),new Vector2d(57, 55),Math.toRadians(225)),groups.moveVerticalSlidesAction(highbasket,autoTolerance),groups.moveGripperAction(JohnsIntake.GripperStates.unclamp),groups.moveIntakeAction(JohnsIntake.IntakeStates.outtake)}))
                .addNext(new Delay(2))
