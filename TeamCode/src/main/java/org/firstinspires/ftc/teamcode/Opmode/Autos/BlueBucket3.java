...
public class BlueBucket3 extends BaseAuto {
    ...
    public void runAuto() {
        ...
        runpath = new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(32, 60), Math.toRadians(270), new Vector2d(58.5, 56.5), Math.toRadians(225)), groups.movePivotAction(PARALLEL), groups.moveVerticalSlidesAction(highbasket, greaterTolerance)})
                ...
                .addNext(new Delay(.5))
                .addNext(new MultipleCommand(groups.moveHorizontalSlides(FULLY_OUT,fullyOutEncoderPos),groups.moveCoAxial(RELEASE),groups.moveIntakeTime(INTAKE,.2)))
                ...
                .addNext(new MultipleCommand(groups.moveCoAxial(RELEASE),groups.moveIntakeTime(OUTTAKE,.2)))
                .addNext(new Delay(.3))
                ...
                .addNext(new MultipleCommand(groups.moveHorizontalSlides(FULLY_OUT,fullyOutEncoderPos),groups.moveCoAxial(RELEASE),groups.movePivot(SHOVELPIVOTPOS),groups.moveIntakeTime(INTAKE,.2)))
                .addNext(new Delay(.3))
                .addNext(new MultipleCommand(groups.movePivot(PARALLEL),groups.moveCoAxial(CLAMP),groups.moveIntakeTime(STOP,.1),groups.moveHorizontalSlides(FULLY_IN,fullyInEncoderPos)))
