...
public class BlueBucket3 extends BaseAuto {
    ...
    public void runAuto() {
        ...
        runpath = new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(32, 60), Math.toRadians(270), new Vector2d(58.5, 56.5), Math.toRadians(225)), groups.movePivotAction(PARALLEL),groups.moveClipMagAction(DOWN), groups.delayAction(.6,groups.moveVerticalSlidesAction(highbasket, greaterTolerance))})
                ...
                .addNext(new MultipleRRActionsWithPathing(new Action[]{groups.moveGripperAction(CLAMP),robot.driveTrain.strafeToLinearHeading(new Vector2d(57, 55), Math.toRadians(225), new Vector2d(58.5,49.7), Math.toRadians(270)), groups.moveIntakeAction(JohnsIntake.IntakeStates.STOP), groups.delayAction(.5, groups.moveVerticalSlidesAction(0, greaterTolerance)), groups.movePivotAction(JohnsIntake.PivotStates.PARALLEL)}))
//                .addNext(new Delay(.5))
...
//                .addNext(new MultipleCommand(groups.moveHorizontalSlides(HorizontalSlides.HorizontalSlideStates.Fully_Out,fullyOutEncoderPos)))
                ...
                .addNext(new MultipleRRActionsWithPathing(new Action[]{robot.driveTrain.strafeToLinearHeading(new Vector2d(58.5,49.7),Math.toRadians(270),new Vector2d(57,56),Math.toRadians(225)),groups.moveVerticalSlidesAction(highbasket,greaterTolerance),groups.movePivotAction(BASKETPOS)}))
                .addNext(new MultipleCommand(groups.moveGripper(UNCLAMP),groups.moveIntake(OUTTAKE)))
                .addNext(new Delay(.3))
                .addNext(new MultipleRRActionsWithPathing(new Action[]{groups.moveGripperAction(CLAMP),robot.driveTrain.strafeToLinearHeading(new Vector2d(57, 56), Math.toRadians(225), new Vector2d(51.5, 43.5), Math.toRadians(305)), groups.moveIntakeAction(JohnsIntake.IntakeStates.STOP), groups.delayAction(.5, groups.moveVerticalSlidesAction(0, greaterTolerance)), groups.movePivotAction(JohnsIntake.PivotStates.PARALLEL)}))
                .addNext(new Delay(.5))
